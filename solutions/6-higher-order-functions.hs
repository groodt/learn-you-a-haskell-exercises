...
-- Create a new higher order method which generalises over the function provided by sumInts (That is, (+) :: Int -> Int -> Int) between a and b
...
higherOrderSequenceApplication start end mapFn reduceFn zero
                               | start > end = zero
                               | otherwise = reduceFn (mapFn start) $ mapFn (higherOrderSequenceApplication (start + 1) end mapFn reduceFn zero)

-- Define a factorial method using the higherOrderSequenceAppliction
hoFactorial :: Int -> Int
hoFactorial x = higherOrderSequenceApplication 1 x id (*) 1
